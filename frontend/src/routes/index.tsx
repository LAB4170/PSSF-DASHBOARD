...
import { PageShell, Section, Kpi, Pill, CHART_COLORS, CHART_PALETTE, Gauge, Funnel } from "@/components/shared/primitives";
import {
...
function useChartTheme() {
  ...
  return {
    tickColor: isDark ? "#94A3C2" : "#4B5A75",
    gridColor: isDark ? "rgba(255,255,255,0.06)" : "rgba(11,17,32,0.08)",
    tooltipBg: isDark ? "#16223C" : "#FFFFFF",
    tooltipBorder: isDark ? "rgba(255,255,255,0.10)" : "rgba(11,17,32,0.12)",
    tooltipLabel: isDark ? "#E6EDF7" : "#0B1120",
    rowBg: isDark ? "rgba(255,255,255,0.04)" : "rgba(11,17,32,0.04)",
  };
}
...
function Dashboard() {
  ...
  const axis = {
    tick: { fill: chart.tickColor, fontSize: 11 },
  ...
  return (
    <PageShell title="Executive Operations Dashboard" subtitle="Live pipeline view across membership, contributions, claims, and finance.">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3">
        <Kpi index={0}  label="Total Members"           value="766,000"    delta="+2.4% MoM"            tone="gold"    icon={<Users className="h-4 w-4" />} />
        <Kpi index={1}  label="Active Members"          value="612,000"    delta="79.9% of total"        tone="teal"    icon={<CheckCircle2 className="h-4 w-4" />} />
        <Kpi index={2}  label="Total Sponsors"          value="412"        delta="+6 this month"          tone="info"    icon={<Building2 className="h-4 w-4" />} />
        <Kpi index={3}  label="Claims Received"         value="1,245"      delta="+82 this week"          tone="gold"    icon={<FileText className="h-4 w-4" />} />
        <Kpi index={4}  label="Claims Paid"             value="712"        delta="57.2% settlement"       tone="success" icon={<Banknote className="h-4 w-4" />} />
        <Kpi index={5}  label="Claims Pending"          value="533"        delta="42 breached SLA"        tone="warning" icon={<Clock className="h-4 w-4" />} />
        <Kpi index={6}  label="Monthly Contributions"   value="KSh 2.1B"   delta="+8.4% MoM"             tone="teal"    icon={<Wallet className="h-4 w-4" />} />
        <Kpi index={7}  label="Contributions Posted"    value="98%"        delta="of received"            tone="success" icon={<TrendingUp className="h-4 w-4" />} />
        <Kpi index={8}  label="Interest Processed"      value="KSh 15B"    delta="FYTD"                   tone="gold"    icon={<Percent className="h-4 w-4" />} />
        <Kpi index={9}  label="Avg Claim TAT"           value="18 d"       delta="Target 14d"              tone="warning" icon={<Clock className="h-4 w-4" />} />
        <Kpi index={10} label="Avg Contribution TAT"    value="6 d"        delta="Within SLA"              tone="success" icon={<Activity className="h-4 w-4" />} />
        <Kpi index={11} label="Under-remitted Sponsors" value="14"         delta="KSh 38.2M arrears"       tone="danger"  icon={<ShieldAlert className="h-4 w-4" />} />
        <Kpi index={12} label="Overpayments"            value="6"          delta="Under review"            tone="warning" icon={<AlertTriangle className="h-4 w-4" />} />
        <Kpi index={13} label="New Members (Mo)"        value="2,140"      delta="vs 1,860 last"           tone="teal"    icon={<Users className="h-4 w-4" />} />
        <Kpi index={14} label="Members Exited (Mo)"     value="640"        delta="-5.1% MoM"               tone="muted"   icon={<Users className="h-4 w-4" />} />
      </div>
        ...
        <Section title="Contributions Trend (KSh B)" className="lg:col-span-2 xl:col-span-3">
          <ResponsiveContainer width="100%" height={260}>
            <AreaChart data={contributionTrend}>
              ...
              <Legend wrapperStyle={{ fontSize: 12, color: chart.tickColor }} />
        ...
        <Section title="Member Distribution">
          <ResponsiveContainer width="100%" height={260}>
            <PieChart>
              ...
              <Tooltip {...tooltipStyle} />
              <Legend wrapperStyle={{ fontSize: 11, color: chart.tickColor }} />
            </PieChart>
          </ResponsiveContainer>
        </Section>
      </div>
        ...
        <Section title="Outstanding Claims by Officer">
          <ul className="space-y-2">
            {outstandingByOfficer.map((o) => (
              <li key={o.officer} className="flex items-center justify-between rounded-md px-3 py-2" style={{ background: chart.rowBg }}>
        ...
        <Section title="Sponsors with Under-remittance">
          <ul className="space-y-2">
            {underRemittance.map((u) => (
              <li key={u.sponsor} className="flex items-center justify-between rounded-md px-3 py-2" style={{ background: chart.rowBg }}>
                <div>
          ...
          <ul className="space-y-3">
            {recentActivity.map((a, i) => (
              <li key={i} className="flex gap-3">
                <div className="mt-1 h-2 w-2 rounded-full bg-[color:var(--gold)] shrink-0" />
                <div>
